...
class Customer(Base):
    ...
    다르면 투입 셀·BOM·협력사 계약이 달라져 컴플라이언스 증빙이 각각 필요해요.
...
class Product(Base):
    ...
    amperage_ah = Column(
        ...
        comment=(
            ...
            "고객사+모델+암페어 조합이 달라지면 컴플라이언스 증빙 대상이 달라지므로 별도 식별 키 역할."
        ),
    )
...
class ProductBrief(BaseModel):
    ...
    # customer_name: Optional[str] = None          # [MARKER] 고객사명(제출 export용)
