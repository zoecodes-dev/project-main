...
- 수동 또는 반자동으로 대외 발송이 완료된 후, 해당 발송 이력(수신처, 발송자, 제목/본문 요약, 발송일시)을 영구 기록하여 규제 당국의 발송 증빙 요구에 대응합니다.
- **엔드포인트**: `POST /dpp/{dpp_id}/deliver`
- 전송 이력은 (다른 도메인의 대외 발송 건과 공용으로 쓰이는) `transmission_logs` 테이블에 적재됩니다. `recipient_type='customer'`, `transmission_type='dpp_report'`로 기록되며, `dpp_id`/제목/본문은 전용 컬럼이 없어 `payload_summary`(TEXT)에 JSON으로 묶어 저장합니다.
- 도달확인(Ack)을 위한 `ack_token`이 매 전송마다 발급되며, 첨부 파일이 추가될 경우 `attachment_urls`(JSONB)에는 S3 key만 저장하고 presigned URL은 저장하지 않습니다(접근 시점에 발급하는 규약).
- 존재하지 않는 사용자의 토큰으로 요청 시 참조 무결성 위반을 방어하도록 설계되어 있습니다.
