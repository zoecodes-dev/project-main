...
| **FEOC** | IRA 우려 외국기업 — 지분 ≥25% 데이터를 위험 점수 산정에 반영 |
...
| `automation`(risk_scoring) | ❌ 결정론 | 규칙 엔진: FEOC 25% 반영 가점식 리스크 점수 |
...
| 제품 | 성격 | 기대 흐름 |
|---|---|---|
| ① BMW iX3 108Ah | **Happy** | 전 단계 통과 → batch_completed |
| ② BMW i4 81Ah | **Gray** | 저신뢰 추출 → low_confidence → HITL |
| ③ Mercedes GLC 94Ah | **Sad** | 신장 원산지 FEOC 위반 + geo 위험 → risk 에스컬레이션 → HITL 반려 |
| ④ Mercedes EQS 118Ah | **Happy** | 전 단계 통과 → batch_completed |
